...
char ACL_get_polar_pan_continuous_vector(API_Data_Polar_Pan_Start *payload) {
  ...
  if (payload->delta_azimuth) {
    vector = payload->delta_azimuth > 0 ? ACL_MAN_POS_BASE_AXIS
                                        : ACL_MAN_NEG_BASE_AXIS;
  } else if (payload->delta_altitude) {
    vector = payload->delta_altitude > 0 ? ACL_MAN_POS_WRIST_PITCH_AXIS
...
int ACL_enqueue_shift_cmd(S_List *cmd_queue, ACL_Axis axis,
  ...
  cmd->len = sprintf(&cmd->payload[0], ACL_SHIFT_FMT, ACL_VAR_POS, axis,
                     encoder_count);
...
/**
 * @brief Helper function for populating a Command Queue with the movements
 * required for a polar pan
