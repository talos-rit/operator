...
#define READ_GPIO_1  4
#define READ_GPIO_2 18
...
void *reader(void *arg) {
   ...
   while (TERM == 0) {
      ...
      if (poll_file_descriptor.revents & POLLIN) {
         ...
         int read_result = read(event_request.fd, &event_data, 1024 * sizeof(struct gpio_v2_line_event));
         ...
         int event_count = read_result / sizeof(event_data[0]);
         printf("Events stored: %d\n", event_count);

         for(int i = 0; i < event_count; i++)
         {
            int idx = -1;
            switch (event_data[i].offset)
            {
               case READ_GPIO_1:
                  idx = 0;
                  break;
            ...
            if (event_data[i].id == GPIO_V2_LINE_EVENT_RISING_EDGE) {
               count[idx].rising++;
               // printf("Rising edge at %llu.\n", event_data.timestamp);
            } else if (event_data[i].id == GPIO_V2_LINE_EVENT_FALLING_EDGE) {
               count[idx].falling++;
            ...
            if (first_event_timestamp == 0) {
               first_event_timestamp = event_data[i].timestamp_ns;
            } else {
               last_event_timestamp = event_data[i].timestamp_ns;
            }
         }
      }
   }

   printf("GPIO 1: Received %llu rising and %llu falling edges.\n",count[0].rising,count[0].falling);
   printf("GPIO 2: Received %llu rising and %llu falling edges.\n",count[1].rising,count[1].falling);
   ...
   __u64 nanos_per_edge = duration / (count[0].rising + count[0].falling);

   printf("Average %llu ns (%llu microseconds) per edge.\n",nanos_per_edge,(nanos_per_edge/1000));

   __u64 per_second = count[0].rising / seconds;
...
int main(int argc, char * const *argv) {
   ...
   event_request.offsets[0] = READ_GPIO_1;
   event_request.offsets[1] = READ_GPIO_2;
   event_request.num_lines = 2;
   event_request.config.flags = GPIO_V2_LINE_FLAG_EDGE_RISING | GPIO_V2_LINE_FLAG_EDGE_FALLING | GPIO_V2_LINE_FLAG_BIAS_PULL_DOWN | GPIO_V2_LINE_FLAG_INPUT;
   event_request.event_buffer_size = 1024;
   ...
   handle_request.config.flags = GPIO_V2_LINE_FLAG_OUTPUT;
   ...
   for (int i = 0; i < 128; i++) {
      ...
      hdata.bits |= hdata.mask;
      res = ioctl(handle_request.fd, GPIO_V2_LINE_SET_VALUES_IOCTL, &hdata);
      if (res == -1) printf("Failed setting line value: (%d) %s\n", errno, strerror(errno));
