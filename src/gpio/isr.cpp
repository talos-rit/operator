...
IchorISR::IchorISR(const char* chip_path) {
  ...
  if (-1 == chip_fd) {
    ...
    LOG_INFO("Successfully opened GPIO device");
}

IchorISR::~IchorISR() { closeISR(); }
...
int IchorISR::openISR() {
  ...
  req.config.flags = 0 | GPIO_V2_LINE_FLAG_BIAS_PULL_DOWN |
...
int IchorISR::RegisterPin(uint8_t pin, GPIO_INTR_TYPE type,
                          GPIO_INTR_TARGET target) {
...
void IchorISR::ProcessEvents() {
  ...
  if (!(poll_event.revents & POLLIN))
    return;  // Poll events weren't the target event

  // Poll was good; Read as much as possible out of the FIFO (impossible to tell
  // how many events are in there before hand)
