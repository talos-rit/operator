 ...
 * Singly Linked List
 *
 * The operation of this module is a little different than the standard
 * implementation of an SLL. Typically, each node in a list stores two things:
 * the next node in the list, and a piece of data For this SLL, a node simply
 * stores the pointer of the next node. The node is a member within the object
 * it tracks To get the pointer to a node's respective object, offset arithmatic
 * is used (DATA_LIST_GET_OBJ in list.h) Doing it this way allows an object to
 * be tracked by multiple lists, and allows the list to be completely agnostic
 * to the data/type it tracks
 */
...
/**
 * @brief Appends the first element of the child list to the last element of the
 * parent list
