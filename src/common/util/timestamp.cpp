...
int8_t UTIL_time_msec_timestamp(char* dst_str, __useconds_t usec) {
  ...
  size_t ret =
      snprintf(dst_str, UTIL_TS_MSEC_LEN, UTIL_TS_MSEC, (usec / 1000) % 1000);
  return ret == 0 ? (size_t)-1 : ret;
}
...
int8_t UTIL_time_msec_usec_timestamp(char* dst_str, __useconds_t usec) {
  ...
  size_t ret =
      snprintf(dst_str, UTIL_TS_MSEC_LEN * 2, UTIL_TS_MSEC UTIL_TS_MSEC,
               (usec / 1000) % 1000, usec % 1000);
  return ret == 0 ? (size_t)-1 : ret;
}
...
int8_t UTIL_time_iso8601_time_offset(char* dst_str, int16_t tz_offset_sec) {
  ...
  size_t ret = snprintf(dst_str, UTIL_TS_OFFSET_LEN, UTIL_TS_OFFSET,
                        tz_offset_sec / (60 * 60), (tz_offset_sec / 60) % 60);
  return ret == 0 ? (size_t)-1 : ret;
}
...
int8_t UTIL_time_iso8601_complete_datetime_msec_UTC(char* dst_str,
                                                    time_t time) {
  char* str_iter = dst_str;
  int8_t ret = iso8601_complete_datetime_UTC(str_iter, time);
  if (ret == -1) {
...
int8_t UTIL_time_iso8601_complete_datetime_msec_local(char* dst_str,
                                                      time_t time) {
  char* str_iter = dst_str;
  int8_t ret = iso8601_complete_datetime_local(str_iter, time);
  if (ret == -1) {
