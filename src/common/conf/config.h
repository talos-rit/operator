 ...
 * Intended to be very easy to extend into child configuration classes that
 * automatically populate the key list, and implement their own
 * accessors/mutators
 */
...
class Config {
 private:
  ...
  /**
   * @brief Linearly searches through keys table and returns the index of the
   * key
 ...
 public:
   ...
   * @details opens the predesignated path, parses it, then closes; YAML is used
   * as the format
   * @returns 0 on success, -1 on failure
   */
  int ParseConfig();
   ...
   * @param deflt Default value of key; Will remain the value in the case of
   * missing key/error
   ...
   * @param key Key to add
   * @param deflt Default value of configuration; (Named to avoid keyword
   * conflict)
  ...
  /**
   * @brief Returns a const char buffer containing the value associated with the
   * given key index
   * @details Uses index to directly access string; Faster than linear search
   * overload
  ...
  /**
   * @brief Copies the associated value of a given key to the destination
   * character pointer
   ...
   * @details Uses linear search to find and access the value, as opposed to
   * direct access with an index
  ...
  /**
   * @brief Gives a detailed list of every added key and their value, and dumps
   * it into the logging system
   * @param log_level Determines the log priority level to print at (e.g.
   * LOG_INFO, LOG_VERBOSE + 2)
   */
