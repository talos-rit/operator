...
static uint8_t get_level_name(char* dst, uint8_t level, bool color) {
  ...
  if (color) offset += sprintf(dst + offset, "%s", _LOG_level_color[level]);
  ...
  if (vlev != -1)
    ...
    offset += sprintf(
        dst + offset, "%-9s ",
        log_level_names[level]);  // Left justified, 13 for "LOG_VERBOSE:0"

  if (color) offset += sprintf(dst + offset, "%s", COLOR_RESET);
...
int8_t LOG_init(const char* log_loc) {
  ...
  if (-1 == the_log.fd)
    LOG_WARN("Invalid log file path: (%d) %s", errno, strerror(errno));
