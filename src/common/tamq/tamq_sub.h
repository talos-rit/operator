/**
 * (Talos) ActiveMQ implementation (TAMQ to avoid confusion with the Apache
 * library) Implementation of the Subscriber interface
 */
#pragma once
...
typedef struct _tamq_config {
  char connection
      [TAMQ_CONFIG_FIELD_LEN]; /** The IPv4 Address and port of the broker (e.g.
                                  'failover:(tcp://127.0.0.1:61616)') */
  char dest_uri[TAMQ_CONFIG_FIELD_LEN]; /** The name of the topic/queue the
                                           client will listen to */
  bool use_topics; /** When true, TAMQ will consume from a topic, rather than a
                      queue */
  bool client_ack; /** When true, TAMQ will not automatically send back an ack
                      after consuming a message from the broker. */
} TAMQ_Configuration;
