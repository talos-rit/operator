...
TEST(ConfigTest, Initialization)
{
    ...
    // Initially, no keys should be present, so retrieval should fail for any key and type
    CHECK_TRUE(config->GetVal("nonexistent_str") == NULL); // Should handle missing keys
    CHECK_EQUAL(-1, config->GetInt("nonexistent_int")); // Should handle missing keys

    // NOTE: There is some interesting behavior here because GetBool is supposed to return a bool
    // The current implementation returns -1 for missing keys, which is not a valid bool value
    // Therefore, the compiler seems to evaluate it as true (non-zero)
    CHECK_EQUAL(-1, config->GetBool("nonexistent_bool")); // Should handle missing keys
}
...
TEST(ConfigTest, ClearKeyVals)
{
    ...
    // Value retrieval should return NULL after clearing since key no longer exists
    CHECK_TRUE(config->GetVal("test_key") == NULL);
    CHECK_EQUAL(-1, config->GetInt("int_key"));
    // GetBool returns -1 for non-existent keys, which is not a valid bool value
    // This is a design choice that may need to be revisited
    CHECK_EQUAL(-1, config->GetBool("bool_key")); // Default false for non-existent key
}
